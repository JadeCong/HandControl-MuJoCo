...
def get_hand_joint_pose(hand_kps):
    ...
    # thumb_ref_plane_normal_vector = np.array([-0.70710678, -0.5, -0.5]) #based on the MPL initial pose plane normal(right direction)
    thumb_ref_plane_normal_vector = np.array([0.85355339, 0.14644661, -0.5]) #based on the MPL initial pose plane normal(right direction)
