"""
Construct the vectors and calculate the angles between vectors for hand joint angles.
"""
import numpy as np
...
#     # cos_theta = np.true_divide(np.dot(v1, v2), (s1 * s2))
...
def calc_plane_normal_vector_from_points(key_point0, key_point1, key_point2):
...
def calc_plane_normal_vector_from_vectors(vector1, vector2):
    # calc the normal vector of a plane based on the two vectors directly
    plane_normal_vector = np.cross(vector1, vector2) #vector1 cross product vector2(vector1 hold to vector2)
...
def calc_vector_projection_onto_vector(vector, vector_ref):
    ...
    vector_projection_onto_vector = np.dot(dot_product / vector_ref_length_square, vector_ref)
